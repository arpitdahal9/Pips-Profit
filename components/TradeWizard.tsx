...
import { X, ChevronRight, ChevronLeft, Upload, Image as ImageIcon, CheckCircle2, Calendar, Wallet, ChevronDown } from 'lucide-react';
...
const TradeWizard: React.FC<TradeWizardProps> = ({ isOpen, onClose }) => {
  ...
  const createTradeFromData = (data: TradeData, index: number = 0): Trade => {
    return {
      ...
      status: data.winLose === 'WIN' ? TradeStatus.WIN : TradeStatus.LOSS,
      pnl: data.winLose === 'WIN' ? data.tpAmount : -data.riskAmount,
