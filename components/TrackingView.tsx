...
const TrackingView = () => {
  const { trades, strategies, updateTrade, accounts } = useStore();
  ...
  const currencySymbol = useMemo(() => {
    ...
    const account = accounts.find(a => a.id === activeTrade.accountId);
    return account?.currencySymbol || '$';
  }, [accounts, activeTrade]);
            ...
            <div className={`p-3 rounded-xl border ${(isEditing ? parseFloat(editForm.pnl) : activeTrade.pnl) >= 0 ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-rose-500/5 border-rose-500/20'}`}>
              ...
              {isEditing ? (
                ...
                <span className={`text-xl font-mono font-bold ${activeTrade.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {activeTrade.pnl >= 0 ? '+' : '-'}{currencySymbol}{Math.abs(activeTrade.pnl).toFixed(2)}
                </span>
              )}
            </div>
